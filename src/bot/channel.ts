...
const SUPPRESSED_API_ERROR_CODES = new Set([
  131005, // wiki.space.getNode "not found" — harmless if a card links to a non-wiki doc
]);
...
export async function startChannel(deps: StartChannelDeps): Promise<BridgeChannel> {
  ...
  channel.on({
    ...
    // Classify common WS errors into the `network` phase so logs can be
    // searched without scanning generic `ws.fail` entries.
    error: (err) => {
...
async function processAgentStream(
  ...
  try {
    for await (const evt of handle.run.events) {
      ...
      // which would leave the for-await waiting forever otherwise.
