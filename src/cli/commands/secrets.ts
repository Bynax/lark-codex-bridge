 ...
 *
 * 2. Any tool implementing openclaw's exec-provider protocol:
 *    `lark-codex-bridge secrets get` reads a JSON-RPC request
 *    from stdin and writes the decrypted secret to stdout. This is what
 *    `accounts.app.secret = { source: "exec", ... }` resolves through when
 *    the bridge stores under ~/.lark-codex/config.json.
 */
