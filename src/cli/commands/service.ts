...
/**
 * Poll `~/.lark-codex/processes.json` for a freshly-registered bridge
...
export async function runServiceStart(opts: ServiceStartOptions = {}): Promise<void> {
  ...
  // Kept as a no-op hook for future preflight checks.
 ...
 * definition file.
 *
 * Idempotent. Leaves ~/.lark-codex/ state untouched (keystore, sessions,
...
export async function runServiceUnregister(): Promise<void> {
  ...
  console.log('  (配置 / 日志 / 会话保留在 ~/.lark-codex/)');
}
