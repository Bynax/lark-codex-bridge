...
const LOG_RETENTION_DAYS = Math.max(
  1,
  Number(process.env.LARK_CODEX_LOG_DAYS ?? process.env.LARK_CHANNEL_LOG_DAYS ?? 7) || 7,
);
 ...
 * the local machine, for example before sharing a log excerpt in chat.
 *
 * Conservative: keeps log structure intact so a reader can still correlate by
