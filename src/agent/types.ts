...
export interface AgentRunOptions {
  ...
  sessionId?: string;
  model?: string;
  /** Local image paths to attach to this run, when the adapter supports it. */
  images?: string[];
  permissionMode?: 'default' | 'acceptEdits' | 'bypassPermissions' | 'plan';
  /**
   * Grace period (ms) between SIGTERM and SIGKILL when stop() is called on
   * the returned run. Lets the agent and any subprocess it spawned clean up
   * before the kernel reaps the tree.
...
export interface AgentRun {
   ...
   *
   * Use this after a terminal stream event (`done` / `error`) to give the
   * process a short clean-exit window before calling stop().
   */
