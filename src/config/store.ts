 ...
 * `~/.lark-codex/secrets-getter` (always user-owned, never a symlink). The
 * wrapper internally `exec`s the real node + bridge with `secrets get`.
 * Bridge itself short-circuits the spawn and reads the keystore directly
 * when it sees its own wrapper path in `command`.
 */
